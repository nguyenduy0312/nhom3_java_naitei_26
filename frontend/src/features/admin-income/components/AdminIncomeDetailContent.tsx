...
export default function AdminIncomeDetailContent({ incomeId }: AdminIncomeDetailContentProps) {
  ...
  const confirmDelete = () => {
    if (window.confirm(`Xóa khoản thu nhập "${income.source}"?`)) {
      deleteMutation.mutate(incomeId, { onSuccess: () => router.push("/admin/incomes") });
    }
  };

  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <Link href="/admin/incomes" className="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-blue-600">
